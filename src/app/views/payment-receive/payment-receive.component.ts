...
@Component({
    selector: 'app-main-payment-receive',
...
export class PaymentReceiveComponent implements OnInit {
    ...
    receiveTransactionList: Array<any> = [];
    ...
    ngOnInit() {
        ...
        this.budget_type_id = this.route.snapshot.paramMap.get("item_type_id");

        if(this.project_id){
            this.filter_project = this.project_id;
            this.onFilterChangeProject(this.project_id);
            if(this.budget_type_id){
                this.filter_category = this.budget_type_id;
            }
    ...
    getTransactionList() {
        ...
        this._service.get('received-transaction-list', params).subscribe(res => {
            this.receiveTransactionList = res.data;
        }, err => { }
        );
    }
